...
class Thread7z(QThread):
    ...
    def run(self):
        ...
        for index, file in enumerate(self.file_dict.keys(), start=1):
            ...
            result = function_archive.subprocess_run_7z('l', file, _PASSWORD_NONE)
            if type(result) is not State7zResult.Success:  # 不成功则用l命令继续测试密码
    ...
    def extract_by_7z(self, file, password, exclude_rules,output_folder=None):
        """使用popen方法调用7z，并实时发送进度信息"""
